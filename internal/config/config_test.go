...
var validBase = map[string]string{
	...
	"HTTP_PORT":                        "8080",
	"DASHBOARD_PORT":                   "3000",
	"WATCHLIST_MAX_SIZE":               "80",
	"WATCHLIST_MIN_VOLUME_USD":         "50000000",
...
func TestLoad_AllFieldsBound(t *testing.T) {
	...
	assert.InDelta(t, 0.06, cfg.OISurge.FromLowPct, 1e-9)
	assert.InDelta(t, 0.04, cfg.OISurge.RecentGrowthPct, 1e-9)
	assert.Equal(t, 11, cfg.OISurge.LookbackPeriods)
	assert.Equal(t, 7, cfg.OISurge.RecentPeriods)
	assert.InDelta(t, 0.6, cfg.OISurge.MinGrowingRatio, 1e-9)

	assert.InDelta(t, 2.5, cfg.SquareHot.Multiplier, 1e-9)
	assert.Equal(t, 65, cfg.SquareHot.LookbackMin)
	assert.Equal(t, 26, cfg.SquareHot.BaselineLookbackHours)

	assert.True(t, decimal.RequireFromString("73.50").Equal(cfg.Position.MarginPerTradeFull), "MarginPerTradeFull")
	assert.True(t, decimal.RequireFromString("36.25").Equal(cfg.Position.MarginPerTradeHalf), "MarginPerTradeHalf")
	...
	assert.InDelta(t, 0.07, cfg.Exit.DisasterStopPct, 1e-9)
	assert.Equal(t, 15, cfg.Exit.ATRPeriod)
	assert.Equal(t, "30m", cfg.Exit.ATRTimeframe)
	assert.InDelta(t, 0.09, cfg.Exit.SignalFailOIDropPct, 1e-9)
	assert.InDelta(t, 0.04, cfg.Exit.SignalFailPriceLowBufferPct, 1e-9)
	assert.InDelta(t, 0.06, cfg.Exit.TPStage1Pct, 1e-9)
	assert.InDelta(t, 0.31, cfg.Exit.TPStage1Ratio, 1e-9)
	assert.InDelta(t, 0.13, cfg.Exit.TPStage2Pct, 1e-9)
	assert.InDelta(t, 0.32, cfg.Exit.TPStage2Ratio, 1e-9)
	assert.InDelta(t, 0.04, cfg.Exit.TrailingActivatePct, 1e-9)
	assert.InDelta(t, 2.5, cfg.Exit.TrailingDistanceATRMult, 1e-9)
	assert.Equal(t, 26, cfg.Exit.SoftTimeoutHours)
	assert.Equal(t, 76, cfg.Exit.HardTimeoutHours)

	assert.InDelta(t, 0.06, cfg.Risk.DailyLossHaltPct, 1e-9)
	assert.Equal(t, 6, cfg.Risk.ConsecutiveLossHaltCount)
	assert.Equal(t, 26, cfg.Risk.ConsecutiveLossHaltHours)
	assert.InDelta(t, 0.04, cfg.Risk.BTCCrashHaltPct, 1e-9)
	assert.Equal(t, 31, cfg.Risk.BTCCrashHaltMinutes)
	assert.InDelta(t, 0.09, cfg.Risk.TotalFloatLossHaltPct, 1e-9)
	assert.Equal(t, 4, cfg.Risk.APIErrorRateLimit)

	assert.Equal(t, 9, cfg.Collector.OIConcurrency)
	assert.Equal(t, 7, cfg.Collector.SquareHashtagConcurrency)
}
